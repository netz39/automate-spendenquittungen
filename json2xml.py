...
def createSingleDonationString(dictOrganization: dict, dictDonor: dict):
    ...
    template = template.replace('$k3', dictOrganization['checkboxWegenFoerderung'])
    template = template.replace('$k4', dictOrganization['checkBoxEinhaltung'])
    template = template.replace('$zwecke1', '\n'.join(dictOrganization['Zwecke']))
    template = template.replace('$zwecke2b2', '\n'.join(dictOrganization['Zwecke']))
    template = template.replace('$stnr2', dictOrganization['SteuerbescheidNummer'])
    template = template.replace('$finamt2', dictOrganization['Finanzamt'])
    template = template.replace('$datum3', dictOrganization['SteuerbescheidDatum'])
    ...
    template = template.replace('$wert1', '{0:.2f}'.format(float(dictDonor['Betrag'])))
...
def createMultiDonationString(dictOrganization: dict, dictDonor: dict):
    ...
    for betrag in dictDonor['Betraege']:
        ...
        betraegeStr = betraegeStr + '\t\t\t\t<element id="ID_LINE">{0}</element>\n'.format(numBetraege)
